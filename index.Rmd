...
For example, we can use `mice()` to give you us initial predictor matrix, and change it afterwards, without running the algorithm. This can be done by setting the maximum number of iterations to `maxit=0`. This leaves the algorithm at initialization. 
```{r, cache = FALSE}
ini <- mice(nhanes, maxit=0, print=F)
pred <- ini$pred
pred
```
The object `pred` contains the predictor matrix from an initial run of `mice` with zero iterations. 
