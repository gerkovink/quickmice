---
title: "`mice`: a quick practical guide"
...
**12. Let us focus more precisely on the missing data patterns. Does the missing data of `gen` depend on `age`? One could for example check this by making a histogram of `age` separately for the cases with known genital stages and for cases with missing genital stages.**
...
**13. Impute the `boys` dataset with mice using all default settings and name the `mids` (multiply imputed data set) object `imp`.**
```{r}
...
**8. Compare the means of the imputed data with the means of the incomplete data.**
...
**14. Get an overview of the data.** 
...
**15. Generate five imputed datasets with the default method `pmm`. Give the algorithm 10 iterations. **
```{r}
...
**16. Perform a regression analysis on the imputed dataset with `sws` as dependent variable and `log10(bw)` and `odi` as independent variables.**
...
**17. Pool the regression analysis and inspect the pooled analysis.**
```{r}
est1 <- pool(fit1)
est1
summary(est1)
...
**18. Impute `mammalsleep` again, but now exclude `species` from the data.**
```{r, cache = FALSE}
...
**19. Compute and pool the regression analysis again. **
```{r}
...
**20. Plot the trace lines for the new imputations**
```{r}
plot(imp2)
```
...
**22. Use passive imputation to impute the deterministic sleep relation in the `mammalsleep` data. Name the new multiply imputed dataset `pas.imp`.**
