---
title: "A one-day crash course in `mice`"
author: "Gerko Vink and Stef van Buuren [![](https://zenodo.org/badge/271731959.svg)](https://zenodo.org/badge/latestdoi/271731959)"
format: 
  html:
    toc: true
---
...
As you can see, we can easily feed the new matrix `pred` to `mice`. We can also abbreviate the logical operator in the argument `print=FALSE`. 
...
```{r}
imp1 <- mice(mammalsleep, maxit = 10, print=F, seed = 123)
```
...
```{r, cache = FALSE}
imp2 <- mice(mammalsleep[ , -1], maxit = 10, print = F, seed = 123)
```
