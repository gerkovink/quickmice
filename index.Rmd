...
**16. Get an overview of the data.** 
...
**17. Generate five imputed datasets with the default method `pmm`. Give the algorithm 10 iterations. **
```{r}
...
**18. Perform a regression analysis on the imputed dataset with `sws` as dependent variable and `log10(bw)` and `odi` as independent variables.**
...
**19. Pool the regression analysis and inspect the pooled analysis.**
```{r}
est1 <- pool(fit1)
est1
summary(est1)
...
**20. Impute `mammalsleep` again, but now exclude `species` from the data.**
```{r, cache = FALSE}
...
**21. Compute and pool the regression analysis again. **
```{r}
...
**22. Plot the trace lines for the new imputations**
```{r}
plot(imp2)
```
...
**23. Use passive imputation to impute the deterministic sleep relation in the `mammalsleep` data. Name the new multiply imputed dataset `pas.imp`.**
